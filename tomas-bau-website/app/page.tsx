    ...
    <>
      ...
      <section className="hero" aria-labelledby="hero-title">
        <div className="hero-photo">
          <Image src="/images/mosaikparkett-versiegelt.jpg" alt="Geschliffenes und versiegeltes Eichen-Mosaikparkett – ein Projekt von Tomas Bau & Sanierung" fill priority sizes="(max-width: 767px) 100vw, 60vw" className="object-cover object-[center_65%]" />
        </div>
        ...
        <div className="container-page relative z-10">
          ...
          <a href="#galerie" className="hero-caption">Einblick in unsere Arbeit <Arrow /><span>Mosaikparkett · geschliffen & versiegelt</span></a>
        </div>
      </section>
      ...
      <div className="border-b border-brand-navy/10 bg-stone-50">
        <ul className="container-page flex flex-col gap-3 py-6 text-sm font-semibold sm:flex-row sm:justify-between sm:gap-6">
          {['Material & Verlegung aus einer Hand', 'Parkett, Vinyl, Laminat & mehr', 'Persönliche Beratung am Telefon'].map((text) => <li key={text} className="flex items-center gap-3"><Check />{text}</li>)}
        </ul>
      </div>

      <section id="leistungen" className="section">
        <div className="container-page">
          <div className="section-heading"><h2 className="h2">Der passende Boden.<br /><span className="text-teal-700">Das richtige Handwerk.</span></h2><p>Vom ersten Rückbau bis zur letzten Leiste: unsere Bodenleger-Leistungen in Wuppertal und Umgebung.</p></div>
          <div className="mt-12 grid gap-8 md:grid-cols-3">
            {[{ slug: 'parkett-verlegen', image: 'fischgraet-parkett-detail.jpg', alt: 'Fischgrätparkett aus Eiche mit natürlicher Maserung' }, { slug: 'vinylboden-verlegen' }, { slug: 'laminatboden-verlegen', image: 'fischgraet-laminat-verlegen-laser.jpg', alt: 'Fischgrät-Laminat wird mit einem Kreuzlinienlaser eingemessen' }].map(({ slug, image, alt }) => {
          ...
          <div className="mt-14 grid border-t border-brand-navy/15 md:grid-cols-2 md:gap-x-12">
            {services.filter((s) => !['parkett-verlegen', 'vinylboden-verlegen', 'laminatboden-verlegen'].includes(s.slug)).map((service) => <details id={`service-${service.slug}`} key={service.slug} className="service-detail"><summary>{service.title}<span className="detail-plus" aria-hidden="true">+</span></summary><p>{service.body}</p></details>)}
          </div>
